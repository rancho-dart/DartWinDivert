...
int main(int argc, char* argv[]) {
	if (argc > 1) {
		if (strcmp(argv[1], "--install") == 0) {
			...
			system("sc create DartWinDivertService binPath= \"C:\\Program Files\\DartWinDivert\\DartWinDivert.exe\" start= auto");
			system("sc description DartWinDivertService \"Dart protocol support service using WinDivert\"");
			system("sc failure DartWinDivertService reset= 0 actions= restart/5000");
			system("sc start DartWinDivertService");
			return 0;
		}
		else if (strcmp(argv[1], "--uninstall") == 0) {
			// Uninstall system service
			system("sc delete DartWinDivertService");
			return 0;
		}
	}
...
void WINAPI ServiceMain(DWORD argc, LPTSTR* argv) {
	//while (!IsDebuggerPresent()) Sleep(100); // If you want to attach debugger, uncomment this line
